...
// Classe Tools pour gérer les outils
class Tools {
  ...
  static selectDiv(div) {
    ...
    // Mettre à jour les paramètres dans l'onglet de droite
    Parameters.updateParameters(selectedDiv);
  
    // Mettre à jour les divs à mettre à jour
...
class Parameters {
  ...
  static rgbToHex(rgb) {
    // Convertir les valeurs RGB en tableau de nombres
    ...
    // Retourner la valeur hexadécimale complète
...
// Fonction AJAX pour envoyer la structure générée au fichier PHP
...
// Classe Root pour la div de base (frame)
class Root {
  constructor() {
  ...
  handleOutputClick(event) {
    ...
    if (currentTool === 'create' && !isCreating && target === parentDiv) {
      ...
      // Mettre à jour les paramètres dans l'onglet de droite pour la nouvelle div créée
...
// Instanciation de la classe Root
const root = new Root();
