<?php
// Fonction récursive pour supprimer les dossiers et fichiers
function recursiveRemoveDir($directory) {
...
$hexValue = $_POST['color'];
$type = $_POST['type'];
$nomDuTest = $_POST['name'];
...
try {
    ...
    foreach ($arbo as $key => $elt) {
        mkdir('./' . $chemin . $key . '/', 0755, true);
        file_put_contents('./' . $chemin . $key . '/' . $elt[0], $elt[1]);
    }
// Créer le fichier ZIP
$zip = new ZipArchive();
$zipFileName = $nomDuTest.'.zip';
if ($zip->open($zipFileName, ZipArchive::CREATE | ZipArchive::OVERWRITE) !== true) {
    throw new Exception('Impossible de créer le fichier ZIP');
}

// Ajouter le dossier racine au fichier ZIP
$zip->addEmptyDir($chemin);

// Récupérer les fichiers à ajouter au ZIP avec leurs chemins relatifs
$files = array_merge(
    glob('./' . $chemin . '*.*'),
    glob('./' . $chemin . 'css/*.*'),
    glob('./' . $chemin . 'js/*.*'),
...
foreach ($files as $file) {
    $relativePath = str_replace('./' . $chemin, '', $file);
    $zip->addFile($file, $chemin . $relativePath);
}
